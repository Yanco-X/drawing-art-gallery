...
import { ApiError, fetchPieces } from '../services';
...
const picksInOrder = (all: Piece[], picked: string[]) => {
  const byId = new Map(all.map((piece) => [piece.id, piece]));
  return picked.flatMap((id) => byId.get(id) ?? []);
};

export interface PiecePicksProps {
  open: boolean;
  onClose: () => void;
  picks: string[];
...
/**
 * Pick pieces from the gallery and put them in order, for a surface that
 * shows a few of them: the spotlight, the about page.
 */
export const PiecePicksDialog = ({
  open,
  onClose,
  picks,
  onSaved,
  ...
  max,
  save,
  slotsFor = picksInOrder,
  emptyHint,
  note,
  nothingToShow,
}: PiecePicksProps & {
  title: string;
  /** Over the ordered list: where the picks will appear. */
  listLabel: string;
  max: number;
  save: (pieceIds: string[]) => Promise<unknown>;
  /** What the list shows: the picks, and whatever the surface fills in. */
  slotsFor?: (all: Piece[], picked: string[]) => Piece[];
  /** In place of the count while nothing is picked. */
  emptyHint: string;
  note?: string;
  /** When the list has nothing in it at all. */
  ...
  const toggle = useCallback(
    (id: string) =>
      setPicked((now) => {
        ...
        // something the owner chose is worse than not adding one more.
  ...
  const slots = useMemo(() => slotsFor(all, picked), [slotsFor, all, picked]);
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[4fr_1fr]">
          ...
          <aside className="flex min-h-0 flex-col gap-5 overflow-y-auto border-t border-line p-6 md:border-t-0 md:border-l">
            <div className="flex flex-col gap-3">
              ...
              {slots.length === 0 && nothingToShow && (
                <p className="text-[13px] text-faint">{nothingToShow}</p>
              )}
            </div>
...
export default PiecePicksDialog;
